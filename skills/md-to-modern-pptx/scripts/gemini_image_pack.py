...
def generate_one(
    *,
    ...
    if model.lower().startswith("gemini-"):
        native_url = f"{base}/v1beta/models/{model}:generateContent"
        aspect_ratio = None
        ...
        for attempt in range(3):
            ...
            if r.status_code < 400:
                resp_json = r.json()
                try:
                    img_bytes, mime = _extract_inline_image_bytes(resp_json)
                except Exception as e:
                    raise RuntimeError(
                        f"Model '{model}' did not return an inline image. "
                        f"Details: {e}"
                    ) from e
                out_path = out_path.with_suffix(_mime_to_ext(mime))
                out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    if args.cmd == "list-models":
        ...
        base = _normalize_base_url(base_url)
        r = requests.get(f"{base}/v1/models", headers=_auth_headers(api_key), timeout=120)
        if r.status_code >= 400:
            snippet = (r.text or "")[:2000]
