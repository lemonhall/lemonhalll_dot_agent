...
- **全局（推荐）**：`~/.agents/skills/md-to-modern-pptx/.env`（一次配置，所有项目可复用）
- **项目级**：`<项目根目录>/.env`（只对当前项目生效；脚本也会自动读）

`.env` 内容参考 `skills/md-to-modern-pptx/.env.example`：

```dotenv
GEMINI_BASE_URL=https://your-gateway.example.com
GEMINI_API_KEY=your_key_here
GEMINI_MODEL=gemini-3-pro-image-preview
```

说明：这里按“中转站/网关”的 **base_url + key** 方式设计（脚本默认走 `POST /v1/images/generations`，并兼容“task_id 轮询”式返回）。
