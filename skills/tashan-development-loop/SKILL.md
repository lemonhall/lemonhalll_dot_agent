...
description: Use when 项目需要 PRD/Spec、版本化执行计划（docs/plan/vN-*）、TDD/E2E、独立 Review Loop、完成信号和可审计追溯来交付中大型功能；尤其适用于担心文档漂移、测试后补、E2E 逃逸、追溯断链、过早宣称 done 的场景。
---
...
- [塔山质量控制层（Review Loop）](#塔山质量控制层review-loop)
- [执行计划：塔山开发循环（Strict）](#执行计划塔山开发循环strict)
- [端到端测试（E2E Gate）](#端到端测试e2e-gate)
- [测试用例编写规范](#测试用例编写规范)
- [回顾差异（愿景 vs. 现实）](#回顾差异愿景-vs-现实)
- [触发可靠性与成本上限](#触发可靠性与成本上限)
- [跨运行记忆与废弃标准](#跨运行记忆与废弃标准)
- [完成信号协议（Stop Hook Friendly）](#完成信号协议stop-hook-friendly)
- [提交与推送（强制）](#提交与推送强制)
- [计划变更处理规则](#计划变更处理规则)
...
塔山项目循环的目标不是"写代码"，而是让项目从愿景出发，经过可执行的计划与里程碑，进入可验证的开发闭环；每一轮交付都要回顾"实现与愿景的差异"，并形成下一版本计划，循环往复，直至愿景与实现之间无阻塞差异、无未处置风险、无证据断链为止。

收敛标准不是"直到 reviewer 找不到任何小意见"，而是：**没有 BLOCKER 级发现；MAJOR 级发现已修复、降级或显式进入下一轮；所有残余风险都有归属和证据。**
...
| 8 | 独立 Review + 回顾差异 | 阻塞发现清零，差异清单写入 `vN-index` | [塔山质量控制层](#塔山质量控制层review-loop) |
...
│     ├── 单元测试 / 集成测试（TDD 驱动）
│     └── 端到端测试（E2E Gate）
├── 7. 独立 Review Loop（最多 3 轮，BLOCKER 清零）
├── 8. 回顾差异 + 跨运行记忆
├── 9. 新版本计划 + 提交推送
└── 10. 重复直到无阻塞差异、无未处置风险、无证据断链
```
...
塔山不是单纯的计划清单；每个里程碑在声称完成前，必须经过一个有边界的 **产出 → 独立检查 → 修复 → 复查** 循环。这个循环只负责发现和压低质量风险，**不得替代最终完成声明**。

### 严重度词汇（强制）

所有 Review 发现必须使用同一套严重度，不允许只写"有问题/没问题"：

| 严重度 | 含义 | 处置 |
|---|---|---|
| **BLOCKER** | 违反 PRD/DoD、测试失败、E2E 断裂、追溯断链、完成信号不合法、安全/数据风险 | 必须修复或用证据证明误报；否则不得宣称完成 |
| **MAJOR** | 影响主要用户路径、可维护性或后续迭代，但不直接破坏当前 DoD | 本轮修复，或写入差异列表并绑定下一轮计划 |
| **MINOR** | 局部清理、命名、文案、低风险边界 | 可携带，不阻塞完成 |
| **NOTE** | 建议、观察、可选优化 | 记录即可 |

完成目标是：**没有未处置的 BLOCKER；所有 MAJOR 都有修复、降级理由或下一轮归属。**

### 独立检查者

Review 必须尽量和 maker 解耦：

1. 使用新鲜上下文检查当前产物、PRD、vN 计划、测试结果和 diff。
2. reviewer 使用对抗式提示：专门寻找 PRD 漂移、DoD 软化、测试后补、E2E 逃逸、追溯断链、完成信号误报。
3. reviewer 不延续 maker 的推理链；不要把 maker 的自我总结当作事实，必须看原文档、命令输出和 diff。
4. 高风险交付可以升级为第二 reviewer 或不同模型族；若做成对比较，必须交换候选顺序，避免位置偏差。
5. 如果只能使用同一个模型，也要在 Review 记录中写明"同模型自评风险"，并依靠后续 catch-rate 审计验证是否退化成橡皮图章。

### 有界迭代

每个里程碑的 Review Loop 默认最多 **3 轮**：

1. 第 1 轮：完整检查 BLOCKER / MAJOR / MINOR / NOTE。
2. 第 2 轮：只复查上轮 BLOCKER、MAJOR 和它们可能引入的回归。
3. 第 3 轮：最后修复轮；仍有 BLOCKER 时，停止宣称完成，把残余风险升级给人类或下一轮计划。

不得为了追求"干净"无限加轮。最后一轮之后只能做两件事：记录残余风险，或明确升级；不能边说"还有问题"边输出完成信号。

### 发现签名与回归

从第 2 轮开始，每条 BLOCKER / MAJOR 都要有稳定签名：

```
<area>::<requirement-or-file>::<failure-mode>
```

示例：

- `traceability::REQ-0001-003::missing-e2e`
- `dod::v2-question-bank::soft-acceptance`
- `completion-signal::v3::premature-done`

如果同一签名连续出现，标记为 **STUCK** 并升级；如果已解决签名重新出现，标记为 **REGRESSION**。STUCK 和 REGRESSION 都是 BLOCKER，除非有明确证据证明误报。

### Review 输出格式

每次 Review 必须留下结构化记录，可写入 `vN-index.md` 的 Review 段，或单独追加到 `docs/plan/tashan-review-log.md`：

```markdown
## Tashan Review - vN / Mx

- reviewer_context: fresh|same-model|human|ci
- round: 1|2|3
- cost_profile: light|standard|strict
...
|---|---|---|---|
| BLOCKER | traceability::REQ-0001-003::missing-e2e | `vN-index` E2E column is empty | fixed in commit abc123 |
```
...
8. **Review Loop**：独立检查 + 修复 BLOCKER / MAJOR + 记录签名、回归和残余风险
...
- Review 记录存在，且写明 reviewer_context、round、cost_profile、verdict。
- 没有未处置 BLOCKER。
- MAJOR 要么已修复，要么有证据降级，要么进入下一轮 `v(N+1)` 计划。
- STUCK / REGRESSION 签名已升级，不得静默丢弃。
- 如果 Review 未触发，必须把原因写进 `vN-index`，并禁止输出完成信号。
...
一个不触发的循环没有价值；一个太贵的循环会被人绕过。塔山必须同时控制这两件事。

### 触发点（强制）

Review Loop 至少绑定以下完成动作之一：

| 触发点 | 最低要求 |
|---|---|
| `vN` 文档写作完成 | 输出 `v_doc_writing_done` 前，检查 PRD / plan / index / 追溯矩阵完整性 |
| 里程碑完成 | 输出 `v_milestone_done` 前，检查 DoD、测试、E2E、证据、差异列表 |
| 整个 `vN` 完成 | 输出 `v_task_fully_done` 前，检查全部里程碑、Review 记录、提交、推送、残余风险 |
| `git commit` / `git push` 前 | 若项目允许，使用 pre-commit / pre-push / CI job 执行文档和追溯检查 |

如果当前环境无法实现 hook，必须在 `vN-index` 的 Review 段写明：
...
如果连续 2 个里程碑出现 `skipped_triggers > 0`，下一轮必须先修触发机制，再继续扩大功能范围。

### 成本档位

每次执行塔山循环前选择一个成本档位，并写入 Review 记录：

| 档位 | 适用 | 上限 | 必跑项 |
|---|---|---|---|
| `light` | 小文档、小配置、低风险改动 | 1 轮 Review，约 10 分钟 | diff + 相关验证 + 完成信号检查 |
| `standard` | 普通功能、里程碑交付 | 最多 3 轮 Review，约 45 分钟 | 单元/集成 + E2E + 追溯矩阵 + Review Loop |
| `strict` | 高风险、发布、数据迁移、安全/支付/权限 | 最多 3 轮 Review，可加第二 reviewer | 全量验证 + 双 reviewer 或人工复核 + 触发审计 |

成本上限耗尽时，不得继续自动加轮；必须把状态标为 `escalated`，记录剩余 BLOCKER / MAJOR，并交给人类或下一轮计划。

---

## 跨运行记忆与废弃标准

塔山的 PRD、ECN、vN-index 是项目记忆；但循环本身也需要记忆，否则没人知道它是否还值得运行。

### Catch-rate 日志

推荐维护追加式日志：`docs/plan/tashan-loop-log.md`。每次 Review 后追加一行：
...
- `blockers_found`：Review 抓到且更便宜检查未抓到的 BLOCKER 数。
- `escaped_defects`：Review 通过后，后续 CI、用户、部署或人工发现的阻塞缺陷数。
- `fixed_by_permanent_check`：被固化为测试、脚本、hook、CI 或文档检查的重复问题数。

### 永久化检查

同类 BLOCKER 第 2 次出现时，优先把它固化成永久检查，而不是继续依赖人记住：

- 需求追溯断链 → 文档检查脚本；
- E2E 缺失 → CI 门禁；
- 完成信号误报 → stop hook / 文本审计；
- 编码乱码 → 乱码扫描；
- 测试后补 → plan 模板和 Review 提问项。

### 废弃 / 缩窄标准

如果连续 5 次标准档 Review 都满足以下条件：

- `blockers_found = 0`
- `escaped_defects = 0`
- 主要风险已被永久检查覆盖

则下一轮可以把相关检查降为 `light`，或把人工 Review 缩窄为抽检。反过来，如果 `escaped_defects > 0`，下一轮必须升级成本档位或补永久检查。

---

## 完成信号协议（Stop Hook Friendly）

如果当前交付是按塔山循环推进的 `vN` 文档、某个 `M` 里程碑，或整个 `vN`，那么在**最终对用户的收尾回复**中，除了自然语言总结外，必须额外输出一段**机器可读完成信号**，供 stop hook / 外部自动化直接读取。

这段信号的目标不是"好看"，而是降低分类器难度、减少误判、让自动化链路优先读协议而不是猜语义。

### 发信号的前提

只有在**当前层级真的完成**时才允许发信号。只要还存在以下任一情况，就**禁止输出完成信号**：

- 还有明确未完成项、下一步待办、阻塞项
- 还有必须补跑但尚未补跑的测试 / smoke / E2E / live API 验证
- 还没有完成对应层级的 Review Loop，或 Review 记录中仍有未处置 BLOCKER
- 触发审计显示本轮应触发但未触发，且没有写明原因和处置
- 还差 `git commit` / `git push` / merge / 回写文档 / 追溯矩阵更新等计划内收尾动作
- 还需要用户或人工介入才能闭合当前 DoD，且该介入尚未完成

一句话：**只差一步，仍然不算 done；不算 done，就不要发 signal。**

### 固定格式（逐字输出）

把下面这段放在最终收尾回复的结尾附近，单独占行，不要改 key 名，不要改枚举值，不要加同义词：
...
- `tashan_version`：当前版本号，如 `v1`、`v23`；未知时不要编造，宁可不发信号。
- `tashan_status`：只能是 `v_doc_writing_done`、`v_milestone_done`、`v_task_fully_done` 三选一。
- `tashan_milestone`：只有 `v_milestone_done` 时填真实里程碑，如 `M2`；其他情况一律填 `none`。
- `tashan_verdict`：当前固定为 `done`；未完成时不要输出整段信号。

### 三种状态的使用规则

- `v_doc_writing_done`
  - 当前 `vN` 所需 PRD / spec / plan / index / 相关文档已按本轮范围写完并自检通过。
  - 这表示"文档写作完成"，**不表示代码或整个 v 已完成**。

- `v_milestone_done`
  - 当前某个 `M` 的 DoD 已满足，且该 `M` 绑定的验证已完成并有证据。
  - 这表示"某个里程碑完成"，**不表示整个 v 已完成**。

- `v_task_fully_done`
  - 当前 `vN` 的全部里程碑都已完成，且本轮要求的验证、回顾、文档同步、交付收尾动作都已闭合。
  - 如果还存在"只差 live smoke / 只差 push / 只差人工确认"之类尾项，**禁止**发这个状态。

### 与自然语言总结的关系

- 自然语言可以自由发挥，但**不得与完成信号矛盾**。
- 如果文字里写了"还没做的一件事是..."、"还差..."、"待补..."、"下一步..."，通常就说明**不该发 signal**。
- 外部 hook / 分类器应优先读取这段信号；只有信号缺失时，才退回自然语言分类。
- 若外部 hook 能读取 `vN-index`，应同时检查 Review verdict、BLOCKER 数、trigger audit 和 git 状态。

### 反例（禁止当作 done）

以下这类收尾回复，哪怕大部分工作都做完了，也**不能**发完成信号：

- "单元测试都过了，**还没做的一件事是 live API smoke**。"
- "实现已经落下来了，**remote 还没配**，后面再推。"
- "文档都写完了，**等你确认后我再开始实现**。"
...
| 9 | Review 逃逸压力 | "我自己已经复盘过了，不需要独立检查。" |
| 10 | 成本逃逸压力 | "塔山太重了，这次先不跑。" |
...
| "只差 live smoke / push / merge，也可以先说 done" | 只差一步也不是 done；**禁止输出完成信号**，先把尾项闭合。 |
...
| "Review 太贵，先跳过" | 改用 `light` 档，而不是静默跳过；跳过率必须记录。 |
| "我自己检查过了" | 自检不是独立 Review；至少要新鲜上下文 + 对抗式检查。 |
| "还有 BLOCKER 但用户能先用" | BLOCKER 未处置就不得输出完成信号；必须修复、证据拒绝或升级。 |
...
| Review 只写"看过了" | 无法审计，也无法复查回归 | 使用结构化 Review 记录和严重度签名 |
| 流程太重就整体跳过 | 触发可靠性崩溃 | 选择成本档位，必要时降级为 `light` |
...
| Review 记录 | `vN-index` 或 `tashan-review-log.md` 是否存在 verdict、严重度、签名和处置 |
| 触发审计 | expected / actual / skipped Review 触发是否记录 |
| 成本档位 | Review 是否声明 `light` / `standard` / `strict`，是否超过上限 |
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.2 | 2026-06-18 | 按 meta-loop 评审补强闭环设计：新增独立 Review Loop、严重度门禁、3 轮上限、发现签名/回归、触发审计、成本档位、跨运行 catch-rate 日志和废弃/缩窄标准 |
| v2.1 | 2026-04-19 | 新增 stop hook 友好的完成信号协议：固定 `TASHAN_COMPLETION_SIGNAL` 输出格式，锁定 `v_doc_writing_done / v_milestone_done / v_task_fully_done` 三种状态，并明确"只差一步不算 done"的发信号禁令 |
