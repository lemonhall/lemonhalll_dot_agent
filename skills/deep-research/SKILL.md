...
## Required Artifacts (Markdown + PDF)

At the end of the research, you MUST generate a pair of files:

1) A Markdown report (`.md`) containing the final response (same structure as above)  
2) A matching PDF (`.pdf`) rendered from that Markdown using the locally installed `md-to-pdf` skill (Chrome/Chromium render)

Prerequisites (once per machine/user):

- Install the skill: `npx skills add steveclarke/dotfiles@md-to-pdf -g -y`
- Install its dependencies: `cd $HOME\.agents\skills\md-to-pdf ; npm install`

### File naming

- Use a safe base name derived from the topic, e.g. `CodeBrain-1-Deep-Research`.
- Output files should be: `<base>.md` and `<base>.pdf`.
- Default output directory: current working directory. If the user asks for a location, follow it.

### How to generate the PDF (PowerShell on Windows)

```powershell
$base = "Topic-Deep-Research"
$mdPath = Join-Path $pwd "$base.md"
$pdfPath = Join-Path $pwd "$base.pdf"

# 1) Write the markdown content to $mdPath (UTF-8)
$markdownContent = @'
# Title
...
...
'@
Set-Content -Encoding UTF8 -Path $mdPath -Value $markdownContent

# 2) Convert to PDF using the installed md-to-pdf skill
node "$HOME\.agents\skills\md-to-pdf\scripts\md-to-pdf.mjs" "$mdPath" "$pdfPath"
```

### If conversion is not possible

If the environment cannot run Node/Chrome (or the `md-to-pdf` skill is missing), still write the `.md` file and clearly state why the PDF could not be generated, including the exact error message if available.
